    ...
    public class GUIControls:MonoBehaviour
    {
        ...
        private void OnGUI()
        {
			...
			if(!rpgCharacterController.isDead)
			{
				...
				if(rpgCharacterMovementController.rpgCharacterState != RPGCharacterState.ClimbLadder && rpgCharacterMovementController.rpgCharacterState != RPGCharacterState.Swim)
				{
					StopCasting();
					NavMesh();
					//Actions.
					if(rpgCharacterController.canAction && !navMeshToggle)
					{
					...
					EndSpecial();
					if(!rpgCharacterController.isSitting && rpgCharacterController.weapon == Weapon.RELAX && isTalking)
					{
		...
		private void Charging()
		{
			...
			{
				if(rpgCharacterWeaponController.HasShield())
				{
		...
		{
			if(rpgCharacterController.canAction && !rpgCharacterController.isBlocking)
			{
		...
		{
			if((rpgCharacterMovementController.canJump || rpgCharacterMovementController.canDoubleJump) && !blockGui && !rpgCharacterController.isBlocking && rpgCharacterController.weapon != Weapon.RELAX)
			{
				if(rpgCharacterMovementController.MaintainingGround())
				{
